  ...
  <div class="min-h-screen bg-gray-200">
    <aside class="relative h-screen inline-flex flex-col justify-between items-center bg-white shadow p-6">
      <nav class=" inline-flex flex-col space-y-2">
        <a v-for="(link, index) in nav" class="flex items-center text-gray-600 py-2 cursor-pointer" :class="[
                 [selected == index ? 'bg-indigo-100 text-indigo-500' : 'hover:bg-gray-100'],
                 [ isEnlarge ? 'pl-2 pr-6 rounded-lg' : 'px-2 rounded-full' ]
      ]" @click="selected = index">
          <span v-html="selected == index ? link.selectedIcon : link.icon" class="w-8 h-8 p-1"
                :class="{ 'mr-4' : isEnlarge }"></span>
          <span v-html="link.text" class="font-medium select-none" v-if="isEnlarge"></span>
        </a>
      </nav>
      <div class="flex items-center w-full bg-gray-100 p-4 rounded-lg" v-if="isEnlarge">
      ...
      <img :src="user.avatar" :alt="user.name" class="w-10 h-10 object-cover rounded-full" v-else>
      <button
...
<!--https://codepen.io/angro/pen/xxZEdrp-->
<script>
export default {
  name: "SidebarNavigationBlock",
  data: {
    selected: 0,
    isEnlarge: true,
    nav: [
